      ...
      <AnimatePresence>
        {open && (
          <motion.div
            ...
            className="fixed bottom-6 right-6 z-50 w-[400px] max-w-[calc(100vw-2rem)] h-[560px] max-h-[calc(100vh-3rem)] rounded-2xl border border-border/60 bg-card shadow-2xl flex flex-col overflow-hidden"
          >
            {/* Header */}
            <div className="flex items-center gap-3 px-4 py-3 border-b border-border/40 bg-card shrink-0">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-[11px] text-muted-foreground">Ask about your projects, tasks & data</p>
              </div>
              <Button
                variant="ghost"
                size="icon"
            ...
            <div ref={scrollRef} className="flex-1 overflow-y-auto p-4 space-y-4">
              {messages.length === 0 && (
                <div className="flex flex-col items-center justify-center h-full text-center gap-3 opacity-60">
                  ...
                  <div className="flex flex-wrap gap-1.5 justify-center mt-2">
                    {["Project summary", "Employee workload", "Overdue tasks", "Budget overview"].map((q) => (
                      <button
                        key={q}
                        onClick={() => { setInput(q); }}
                        className="text-[11px] px-2.5 py-1.5 rounded-lg border border-border/60 bg-accent/30 text-muted-foreground hover:bg-accent hover:text-foreground transition-colors"
                      >
                        {q}
                      </button>
                    ))}
              ...
              {messages.map((msg, i) => (
                <div key={i} className={`flex gap-2.5 ${msg.role === "user" ? "justify-end" : "justify-start"}`}>
                  ...
                  {msg.role === "user" && (
                    <div className="h-7 w-7 rounded-lg bg-primary/80 flex items-center justify-center shrink-0 mt-0.5">
            ...
            <div className="p-3 border-t border-border/40 bg-card shrink-0">
              <div className="flex items-end gap-2">
                <textarea
                  ...
                  rows={1}
                  className="flex-1 resize-none bg-accent/30 rounded-xl border border-border/40 px-3 py-2.5 text-sm placeholder:text-muted-foreground/50 focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary/30 transition-all min-h-[40px] max-h-[100px]"
                />
                <Button
                  size="icon"
